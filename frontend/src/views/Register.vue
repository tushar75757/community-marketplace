<template>
  <div class="min-h-[80vh] flex items-center justify-center px-6 py-12">
    <div class="bg-gray-900 w-full max-w-md p-10 rounded-3xl border border-white/5 shadow-2xl space-y-8 relative overflow-hidden">
      ...
      <div class="absolute -bottom-24 -left-24 w-48 h-48 bg-indigo-600/10 blur-3xl rounded-full"></div>
      
      <div class="text-center space-y-2 relative">
        <h2 class="text-3xl font-black text-white tracking-tight">Create Account</h2>
        <p class="text-gray-500 font-medium">Join our community and start trading today.</p>
      </div>

      <form @submit.prevent="handleRegister" class="space-y-6 relative">
        <div class="space-y-2">
          <label class="block text-sm font-bold text-gray-400 ml-1">Full Name</label>
          <input 
            v-model="name" 
            type="text" 
            required 
        ...
        <div class="space-y-2">
          <label class="block text-sm font-bold text-gray-400 ml-1">Password</label>
          <input 
            v-model="password" 
            type="password" 
            required 
      ...
      <p class="text-center text-gray-500 font-medium">
        Small step for you, huge leap for your closet.
      </p>

      <p class="text-center text-gray-500 font-medium pt-4 border-t border-white/5">
        Already have an account? <router-link to="/login" class="text-blue-500 font-bold hover:underline decoration-2 underline-offset-4 transition-all">Login here</router-link>
      </p>
    </div>
  </div>
</template>
